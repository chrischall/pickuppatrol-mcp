 ...
 *
 * The read tools in this server hand back PickUp Patrol's payload close to
 ...
 * honestly say which of PickUp Patrol's fields matter and which are noise.
 *
 ...
 */
export const PUP_VIEWS = ['compact', 'full'] as const;

const NOTE =
  'compact strips image/avatar URLs from the response; "full" returns PickUp Patrol\'s payload untouched. ' +
  'No field projection: this server has no verified record of which PickUp Patrol fields matter, and inventing ' +
  'one would risk dropping a field a caller needs.';

/** The `view` parameter every read tool in this server takes. */
export const viewArg = (): ReturnType<typeof viewParam> => viewParam(PUP_VIEWS, { note: NOTE });
...
export function viewResponse(view: string | undefined, data: unknown): ReturnType<typeof minifiedResult> {
  const rung: View = resolveView(view, PUP_VIEWS);
